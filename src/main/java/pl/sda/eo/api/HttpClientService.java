...
public class HttpClientService <T> {
    ...
    public T getInformations(String url, Class<T> responseClass, Function<JsonElement, LocalDateTime> localDataTimeMapper) {
...
//							return LocalDateTime.ofInstant(Instant.ofEpochMilli(dateTimeJson), ZoneId.systemDefault());
                            return localDataTimeMapper.apply(json);
                        }
                    }
            ).create();
